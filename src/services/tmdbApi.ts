...
import { ExternalIds } from "../Types/common";
...
const tmdbApi = {
  ...
  getSimilar: <T>(
    ...
    const url = `${media_type}/${id}/similar`;
    return axiosInstance.get<Paginate<T>>(url, { params });
  },
  ...
  getEpisodes: <T>(tv_id: number, season_number: number) => {
    const url = `tv/${tv_id}/season/${season_number}`;
    return axiosInstance.get<T>(url);
  },
  getExternalIds: (media_type?: TmdbMediaType,tv_id?: number) => {
    const url = `${media_type}/${tv_id}/external_ids`;
    return axiosInstance.get<ExternalIds>(url);
  }
};
