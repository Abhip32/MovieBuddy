        ...
        <div className={`hero-slide`} style={{ backgroundImage: `url(${originalImage(movie.backdrop_path)})` }}>
            <Wrapper className='h-full relative z-10'>
                <div className="slide-content w-full md:w-[65%] pr-6 ">
                    <Link to={`${urlMap[movie.media_type]}/${encodeURIComponent((movie.name || movie.title).toLowerCase()).replace(/%20/g, '-') || "na"}/${movie.id}`} className="movie-name duration-300 transition-colors hover:text-dark-purple text-3xl md:text-4xl  text-white font-bold  drop-shadow-lg block pr-6">{movie.name || movie.title}</Link>
                        ...
                        <div className="cate">
                            {
                                genres?.map(item => {
                                    return (
                                        <a href="#" key={item.id.toString()} className='cates inline-block mr-3 text-small text-white/60 hover:text-white transition-colors duration-300 ease-out'>{item.name}</a>
                                    )
                                })
                            }
                    ...
                    <div className="movie-desc hidden  sm:block mt-4 text-white font-small text-sm">
                        {movie.overview}
                    </div>
                    <div className="buttons mt-8 flex gap-6">
                        <Link to={`${urlMap[movie.media_type]}/${encodeURIComponent((movie.name || movie.title).toLowerCase()).replace(/%20/g, '-') || "na"}/${movie.id}`} className="watch-btn banner-btn  border-dark-purple text-white bg-dark-purple hover:text-white ">
                            <BsFillPlayFill size={20} /> Watch now
                        </Link>
                        <button onClick={() => onClickTrailer && onClickTrailer(movie.media_type, movie.id)} className="add-btn banner-btn  border-white text-white hover:bg-white hover:text-black"><BiMoviePlay size={16} /> Trailer
                        </button>
                    </div>
                </div>
            </Wrapper>
