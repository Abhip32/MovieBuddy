    ...
    return (
        <div className={classNames('hidden [&.open]:flex  fixed items-center justify-center w-full h-[100vh] bg-black/80 text-white z-50 top-0 left-0', { open: modalState?.isOpen })}>
            <div className='w-full px-4 sm:w-[55%]  '>
                <div className='sm:py-1 bg-black-2 rounded-t-3xl text-white text-right text-2xl'><button onClick={() => modalState?.close()} className='inline-block mr-4 mt-2 hover:text-dark-purple transition-colors'><AiOutlineClose /></button></div>
