...
const Dropdown = ({ buttonIcon, title, renderItems, grid, dropContentClassName, selected = 'All' }: Props) => {
    ...
    return (
        <div className="relative w-[48%] xs:w-auto" >
            <button
                ref={buttonRef}
                className={classNames(`flex items-center gap-1 whitespace-nowrap rounded bg-white px-4 py-2 text-xs 
                leading-normal text-[#4b5156] transition duration-150 ease-in-out 
                focus:outline-none focus:ring-0 active:bg-primary-700 
                capitalize
                font-normal hover:bg-zinc
                w-full
                justify-center
                xs:justify-start
                ...
                {buttonIcon}
                <span>{title}</span>
                <span className='text-black/70'>{selected}</span>
            </button>
            <div
                ref={dropContentRef}
...
export const DropItem = ({ id, name, value, check, onChangeEvent }: DropItemsProps) => {
    ...
    return (
        <div className='flex items-center w-[48%] sm:w-[32%] md:w-[24%]'>
            ...
            <label htmlFor={`${id}`} className='flex h-8 gap-2 peer-checked:px-0 peer-checked:gap-0 px-2 w-full items-center text-black/60 text-xs hover:bg-zinc
                            py-[2px] transition duration-300 rounded cursor-pointer
                            before:content-[""] before:w-3 before:h-3 before:bg-[#434b53]'>
                <BsCheck className='text-dark-purple text-[28px] hidden icons' />
                {name}
            </label>
        </div>
    )
...
export const DropItemSimple = ({ id, name, value, check, fieldName, onChangeEvent }: DropItemsSimpleProps) => {
    ...
    return (
        <div className='flex items-center w-full xs:w-max'>
            ...
            <label htmlFor={`${id}`} className='flex h-8 gap-2 peer-checked:px-2 peer-checked:gap-0 px-4 w-full items-center text-black/60 text-xs hover:bg-zinc
                            py-[2px] transition duration-300 rounded cursor-pointer
                            before:content-[""] before:w-3 before:h-3 before:rounded-full before:bg-white'>
                <BsCheck className='text-dark-purple text-[28px] hidden icons' />
                {name}
            </label>
        </div>
